...
title("SP-BFGS Optimality Gap", fontsize=22)
for r in 1:total_runs
	axes_opt_gap.plot(1:pltIter,log.(10,all_f_vals[:,r]), alpha=0.8, color=color_map(r/total_runs), label="SP-BFGS")
end
...
title("SP-BFGS Gradient Norm", fontsize=22)
all_grad_norms = zeros(maxIter,total_runs)
for r in 1:total_runs
	for j in 1:maxIter
		all_grad_norms[j,r] = norm(objFunTrue.grad_eval(all_x_vals[j,:,r]))
	end
	axes_grad_norm.plot(1:pltIter,log.(10,all_grad_norms[:,r]), alpha=0.8, color=color_map(r/total_runs), label="SP-BFGS")
end
...
title("SP-BFGS Condition Number", fontsize=22)
for r in 1:total_runs
	axes_scaled_Hessian.plot(1:pltIter,log.(10,all_scaled_hess_cond_vals[:,r]), alpha=0.8, color=color_map(r/total_runs), label="SP-BFGS")
end
